    ...
    await withDb(async () => {
        ...
        if (config.toggles.loadUsers) {
            ...
            const prepared = await Promise.all(
                users.map(async (u) => ({
                    ...
                    roles: Array.isArray(u.roles) && u.roles.length ? u.roles : config.defaults.roles,
                }))
            );
        ...
        if (config.toggles.loadDriverDetails) {
            ...
            const prepared = details.map((d) => {
                ...
                if (!userId) {
                    throw new Error(`DriverDetails: could not resolve user for ${d.userEmail || d.userId}`);
                }
                ...
                return {
                    ...
                    service: d.service
                        ? {
                            ...d.service,
                            homeCoordinates: d.service.homeCoordinates ? fixPoint(d.service.homeCoordinates) ?? null : null,
                        }
                        : undefined,
                };
            });

            const inserted = await DriverDetails.insertMany(prepared, { ordered: false });
            if (config.options.logInserted) console.log(`🚕 Inserted ${inserted.length} driver details`);
        }
