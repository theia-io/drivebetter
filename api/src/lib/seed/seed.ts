...
const fixPoint = (p?: any) =>
  p && p.coordinates?.length === 2
    ? {
        type: "Point" as const,
        coordinates: [Number(p.coordinates[0]), Number(p.coordinates[1])] as [
          number,
          number
        ],
      }
...
async function run() {
  ...
  const rawCfg = JSON.parse(await fs.readFile(cfgPath, "utf-8")) as
    | OldConfig
    | NewConfig;
  ...
  await withDb(async () => {
    ...
    if (config.toggles.loadUsers) {
      ...
      const prepared = await Promise.all(
        users.map(async (u) => ({
          ...u,
          passwordHash:
            u.passwordHash || (await hashPassword(config.defaults.password)),
          roles:
    ...
    const usersByEmail = new Map(
      usersAll.map((u) => [u.email.toLowerCase(), u])
    );
    ...
    if (config.toggles.loadDriverDetails) {
      ...
      const prepared = details.map((d) => {
        ...
        if (!userId) {
          throw new Error(
            `DriverDetails: could not resolve user for ${
    ...
    if (config.toggles.loadRides) {
      ...
      const prepared = rides.map((r) => {
        const assignedDriverId = r.assignedDriverId
          ...
          ? usersByEmail.get(String(r.assignedDriverEmail).toLowerCase())
              ?._id || null
          : null;
        ...
        const queue: Types.ObjectId[] = Array.isArray(r.queueDriverEmails)
          ? (r.queueDriverEmails
              .map(
                (e: string) => usersByEmail.get(String(e).toLowerCase())?._id
              )
              .filter(Boolean) as Types.ObjectId[])
