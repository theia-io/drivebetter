...
const RideClaimSchema = new Schema<IRideClaim>(
    {
        ...
        status:   { type: String, enum: ["queued","approved","rejected","withdrawn"], default: "queued", index: true },
    },
    { timestamps: true }
);

RideClaimSchema.index({ rideId: 1, driverId: 1, status: 1 }, { unique: true, partialFilterExpression: { status: "queued" } });
