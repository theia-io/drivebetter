                            ...
                            <CardBody className="p-4 sm:p-6">
                                ...
                                <Grid3>
                                    ...
                                    <NumberField
                                        ...
                                        onChange={(v) => setValues(sv(values, 'capacity.luggageCapacityLiters', v))}
                                    />
                                </Grid3>
                            </CardBody>
                        </Card>
...
function toInt(x: any): number | undefined { const n = Number(x); return Number.isFinite(n) ? n : undefined }
function toFloat(x: any): number | undefined { const n = Number(x); return Number.isFinite(n) ? n : undefined }
function toDate(x: any): string | undefined { if (!x) return undefined; const d = new Date(x); return isNaN(d.getTime()) ? undefined : d.toISOString() }
function splitCSV(s?: string): string[] | undefined { if (!s) return []; return s.split(',').map(t => t.trim()).filter(Boolean) }
function dateInput(iso?: string | null): string | undefined {
    if (!iso) return undefined
    const d = new Date(iso); if (isNaN(d.getTime())) return undefined
