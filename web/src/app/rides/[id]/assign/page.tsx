...
function RatingStars({ value = 0 }: { value?: number }) {
    ...
    return (
        <span
            className="font-medium tracking-tight text-yellow-600"
...
export default function AssignDriverPage() {
    ...
    const [luggageLiters, setLuggageLiters] = useState<number>(
        Number(sp.get("luggageLiters") || 0)
    );
    const [vehicleType, setVehicleType] = useState<VehicleType | "">(
        (sp.get("vehicleType") as VehicleType) || ""
    );
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <div className="flex flex-wrap items-center justify-between gap-2">
                                                <div className="min-w-0">
                                                    <div className="text-gray-900 font-semibold truncate">
                                                        {(user as any).name ||
                                                            `Driver ${String(d.userId).slice(-6)}`}
                                                    </div>
                                                ...
                                                {stats && (
                                                    <div className="flex items-center gap-2 text-sm">
                                                        ...
                                                        {typeof stats.completedRides ===
                                                            "number" && (
                                                            <span className="text-xs text-gray-600">
                                                                Rides: {stats.completedRides}
                                                            </span>
                                                        )}
                                                    </div>
                                                )}
                                            ...
                                            <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-1.5">
                                                        {prefs.airportPermit && (
                                                            <Tag>Airport permit</Tag>
                                                        )}
                                                        {prefs.longDistance && (
                                                            <Tag>Long distance</Tag>
                                                        )}
                                                        {!prefs.airportPermit &&
                                                            !prefs.longDistance && (
                                                                <span className="text-gray-400">
                    ...
                    {!loading && !error && eligible.length === 0 && (
                        <Card variant="elevated">
                            <CardBody className="p-4 text-sm text-gray-600">
                                No results yet. Adjust filters and click{" "}
                                <em>Find eligible drivers</em>.
                            </CardBody>
                        </Card>
                    )}
                </div>
