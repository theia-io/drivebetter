...
import { Filter, ChevronDown, ChevronUp, SlidersHorizontal, ArrowUpDown } from "lucide-react";
...
export default function RidesFilters({
    ...
    const statusSummary =
        statusFilter === "all" ? "All statuses" : getStatusLabel(statusFilter as RideStatus);

    const dateSummary = !dateFrom && !dateTo ? "Any date" : `${dateFrom || "…"} → ${dateTo || "…"}`;

    const sortSummary = getSortLabel(sortBy);

    const activeFilterCount =
        (statusFilter !== "all" ? 1 : 0) + (dateFrom || dateTo ? 1 : 0) + (pendingOnly ? 1 : 0);
            ...
            {filtersOpen && (
                <div className="border-t border-gray-200 px-3 py-3 sm:px-4 sm:py-4 space-y-3">
                    ...
                    <div ref={statusDropdownRef} className="relative">
                        <div className="mb-1 text-[11px] sm:text-xs text-gray-600">Status</div>
                        <button
                            type="button"
                            ...
                            <div className="absolute z-40 mt-1 w-full rounded-lg border border-gray-200 bg-white shadow-lg">
                                ...
                                <ul className="max-h-64 overflow-y-auto px-2 py-2 text-[11px] sm:text-xs">
                                    {STATUS_FILTERS.map((value) => {
                                        ...
                                        if (value === "all") {
                                            return (
                                                <li key="all" className="px-1 py-1">
                                                    <button
                                                        type="button"
                                        ...
                                        const pillClass = getPillStatusColor(value as RideStatus);
                                        const dotClass = getStatusDotColor(value as RideStatus);

                                        return (
                                            <li key={value} className="px-1 py-1">
                                                <button
                                                    type="button"
                                                    className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left ${
                                                        active
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        <span className="capitalize">
                                                            {getStatusLabel(value as RideStatus)}
                                                        </span>
                                                    </span>
                                                </button>
                                            </li>
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-xs sm:text-sm">
                        <label className="flex items-start gap-2 cursor-pointer">
                            <input
                                ...
                                checked={pendingOnly}
                                onChange={(e) => onChangePendingOnly(e.target.checked)}
                            />
                            <div className="flex flex-col">
                                <span className="font-medium text-gray-900">Pending driver</span>
                                <span className="text-[10px] sm:text-[11px] text-gray-500">
                    ...
                    <div ref={sortDropdownRef} className="relative">
                        <div className="mb-1 text-[11px] sm:text-xs text-gray-600">Sort by</div>
                        <button
                            type="button"
                        ...
                        {sortDropdownOpen && (
                            <div className="absolute z-40 mt-1 w-full rounded-lg border border-gray-200 bg-white shadow-lg">
                                <ul className="py-1 text-[11px] sm:text-xs">
                                    {(
                                        ["dateDesc", "dateAsc", "status", "amountDesc"] as SortKey[]
                                    ).map((option) => {
