...
const isJsonContent = (headers: Record<string, string>) =>
    Object.entries(headers).some(
        ([k, v]) => k.toLowerCase() === "content-type" && v.includes("application/json")
    );
...
async function request<TResp = unknown, TBody = unknown>(
    ...
    if (opts.body instanceof FormData) {
        ...
        if (!headers["Content-Type"]) headers["Content-Type"] = opts.body.type || "application/octet-stream";
        init.body = opts.body;
    }
...
export function downloadBlob(path: string, query?: Query) {
    return request<Blob, void>("GET", path, { query, headers: { Accept: "*/*" } });
}

export function uploadForm<T>(path: string, form: FormData, opts?: Omit<HttpOptions<FormData>, "body">) {
    return request<T, FormData>("POST", path, { ...(opts as HttpOptions<FormData>), body: form });
}
